...
int SSL_CTX_check_enc_private_key(const SSL_CTX *ctx)
{
    ...
    if ((ctx->cert->pkeys[SSL_PKEY_SM2_ENC].x509 == NULL) || (ctx->cert->pkeys[SSL_PKEY_SM2_ENC].privatekey == NULL))
    {
        SSLerr(SSL_F_SSL_CTX_CHECK_ENC_PRIVATE_KEY, SSL_R_NO_CERTIFICATE_ASSIGNED);
        return 0;
    }

    return X509_check_private_key(ctx->cert->pkeys[SSL_PKEY_SM2_ENC].x509, ctx->cert->pkeys[SSL_PKEY_SM2_ENC].privatekey);
}
...
int SSL_check_enc_private_key(const SSL *ssl)
{
    ...
    if ((ssl->cert->pkeys[SSL_PKEY_SM2_ENC].x509 == NULL) || (ssl->cert->pkeys[SSL_PKEY_SM2_ENC].privatekey == NULL))
    {
        SSLerr(SSL_F_SSL_CHECK_ENC_PRIVATE_KEY, SSL_R_NO_CERTIFICATE_ASSIGNED);
        return 0;
    }

    return X509_check_private_key(ssl->cert->pkeys[SSL_PKEY_SM2_ENC].x509, ssl->cert->pkeys[SSL_PKEY_SM2_ENC].privatekey);
}
#endif
...
const char *SSL_get_servername(const SSL *s, const int type)
{
    ...
    if (server) {
        ...
        if (SSL_in_before(s)) {
            ...
            if (!SSL_IS_TLS13(s) && !SSL_IS_DTLS13(s) && s->hit && s->session->ext.hostname != NULL)
                return s->session->ext.hostname;
        }
    }
...
#endif
    sm2_sign = pvalid[SSL_PKEY_SM2] & CERT_PKEY_VALID;
...
int ssl_check_srvr_sm2_cert_and_alg(X509 *x, SSL *s)
{
    if (s->s3->tmp.new_cipher->algorithm_auth & SSL_aSM2DSA) {
        /* key usage, if present, must allow signing */
        if (!(X509_get_key_usage(x) & X509v3_KU_DIGITAL_SIGNATURE)) {
            SSLerr(SSL_F_SSL_CHECK_SRVR_SM2_CERT_AND_ALG,
                   SSL_R_SM2_CERT_NOT_FOR_SIGNING);
            return 0;
        }
    }
    return 1;                   /* all checks are ok */
...
void ssl_update_cache(SSL *s, int mode)
{
    ...
    if ((i & mode) != 0
        && (!s->hit || SSL_IS_TLS13(s) || SSL_IS_DTLS13(s))) {
        /*
...
void ssl_clear_cipher_ctx(SSL *s)
{
    ...
    if (s->enc_read_sn_ctx != NULL) {
        EVP_CIPHER_CTX_free(s->enc_read_sn_ctx);
        s->enc_read_sn_ctx = NULL;
    }
    ...
    if (s->enc_write_sn_ctx != NULL) {
        EVP_CIPHER_CTX_free(s->enc_write_sn_ctx);
        s->enc_write_sn_ctx = NULL;
    }
