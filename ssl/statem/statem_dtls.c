...
void dtls1_fix_message_header(SSL *s, size_t frag_off,
                                     size_t frag_len, size_t msg_len);
unsigned char *dtls1_write_message_header(SSL *s, unsigned char *p);
...
int dtls1_do_write(SSL *s, int type)
{
    ...
    while (s->init_num > 0) {
        ...
        if (type == SSL3_RT_HANDSHAKE) {
            ...
            dtls1_fix_message_header(s, frag_off, len - DTLS1_HM_HEADER_LENGTH, 0);
        ...
        if (ret <= 0) {
             ...
             * updated again later.
             */
            dtls1_fix_message_header(s, frag_off, 0, 0);
        }
    }
    return 0;
}
...
int dtls1_buffer_message(SSL *s, int is_ccs)
{
    ...
    if (is_ccs) {
        if (2 == is_ccs) {
            is_ccs = 0;
            if (!ossl_assert(s->d1->w_msg_hdr.msg_len == (unsigned int)s->init_num))
              return 0;
...
void
dtls1_fix_message_header(SSL *s, size_t frag_off, size_t frag_len, size_t msg_len)
{
    struct hm_header_st *msg_hdr = &s->d1->w_msg_hdr;
...
int dtls1_set_handshake_header(SSL *s, WPACKET *pkt, int htype)
{
    ...
    if (htype == SSL3_MT_CHANGE_CIPHER_SPEC) {
        ...
        s->d1->handshake_write_seq = s->d1->next_handshake_write_seq;
        dtls1_set_message_header_int(s, DTLS13_RT_ACK, 0,
                                     s->d1->handshake_write_seq, 0, 0);
    } else {
...
int dtls1_close_construct_packet(SSL *s, WPACKET *pkt, int htype)
{
    ...
    if ((htype != SSL3_MT_CHANGE_CIPHER_SPEC && htype != DTLS13_MT_ACK && !WPACKET_close(pkt))
            || !WPACKET_get_length(pkt, &msglen)
            || msglen > INT_MAX)
        return 0;

    if (htype != SSL3_MT_CHANGE_CIPHER_SPEC) {
        if (htype == DTLS13_MT_ACK) {
            s->d1->w_msg_hdr.msg_len = msglen;
            s->d1->w_msg_hdr.frag_len = msglen;
        } else {
