...
int dtls1_process_record(SSL *s, DTLS1_BITMAP *bitmap)
{
    ...
    int dtls13_unified_hdr_read = SSL_IS_DTLS13(s) && s->enc_read_ctx != NULL;
     ...
     */
    rr->input = &(RECORD_LAYER_get_packet(&s->rlayer)[dtls13_unified_hdr_read ? 
                  SSL3_RECORD_get_dtls13_unified_hdr_len(rr) : DTLS1_RT_HEADER_LENGTH]);
    ...
    if (dtls13_unified_hdr_read) {
        size_t end;

        if (rr->length == 0
                    || rr->type != SSL3_RT_APPLICATION_DATA) {
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_DTLS1_PROCESS_RECORD,
                        SSL_R_BAD_RECORD_TYPE);
            return 0;
        }

        /* Strip trailing padding */
        for (end = rr->length - 1; end > 0 && rr->data[end] == 0;
                    end--)
          continue;
        ...
        if (s->msg_callback)
          s->msg_callback(0, s->version, SSL3_RT_INNER_CONTENT_TYPE,
                      &rr->data[end], 1, s, s->msg_callback_arg);
    }
...
int dtls1_get_record(SSL *s)
{
    ...
    int dtls13_unified_hdr_read = SSL_IS_DTLS13(s) && s->enc_read_ctx != NULL;
    size_t unified_hdr_len, unified_hdr_L;
    unsigned long unified_hdr_epoch;
 ...
 again:
    ...
    if ((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
        (RECORD_LAYER_get_packet_length(&s->rlayer) < 
         (dtls13_unified_hdr_read ? (size_t)DTLS13_UNIFIED_HDR_BITS_LEN : (size_t)DTLS1_RT_HEADER_LENGTH))) {
        rret = ssl3_read_n(s, dtls13_unified_hdr_read ? DTLS13_UNIFIED_HDR_BITS_LEN : DTLS1_RT_HEADER_LENGTH,
        ...
        if (RECORD_LAYER_get_packet_length(&s->rlayer) !=
                    (dtls13_unified_hdr_read ? DTLS13_UNIFIED_HDR_BITS_LEN : DTLS1_RT_HEADER_LENGTH)) {
            RECORD_LAYER_reset_packet_length(&s->rlayer);
            goto again;
        }
        ...
        if (dtls13_unified_hdr_read)
        {
            if (!dtls13_get_unified_hdr(s, *p, &unified_hdr_len, &unified_hdr_epoch)) {
                RECORD_LAYER_reset_packet_length(&s->rlayer);
                goto again;
            }

            /* Epoch */
            rr->epoch = ((~DTLS13_UNIFIED_HDR_E_MASK & (DTLS_RECORD_LAYER_get_r_epoch(&s->rlayer))) | unified_hdr_epoch);
            
            /* read left unified hdr */
            rret = ssl3_read_n(s, unified_hdr_len - DTLS13_UNIFIED_HDR_BITS_LEN, 
                        unified_hdr_len - DTLS13_UNIFIED_HDR_BITS_LEN, 1, 1, &n);
            if (rret <= 0 || n != unified_hdr_len - DTLS13_UNIFIED_HDR_BITS_LEN) {
                return rret;
            }
          
            if (!dtls13_get_unified_hdr_L(s, &unified_hdr_L, p, unified_hdr_len)) {
                RECORD_LAYER_reset_packet_length(&s->rlayer);
                goto again;
            }
            rr->length = unified_hdr_L;

            SSL3_RECORD_set_dtls13_unified_hdr(rr, p);
            SSL3_RECORD_set_dtls13_unified_hdr_len(rr, unified_hdr_len);
        ...
        if (s->msg_callback)
            s->msg_callback(0, 0, SSL3_RT_HEADER, p, 
                        dtls13_unified_hdr_read ? unified_hdr_len : DTLS1_RT_HEADER_LENGTH,
