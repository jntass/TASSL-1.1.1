                             ...
                             unsigned char *out, size_t outlen, int fatal)
{
	...
	static const unsigned char label_prefix_dtls[] = "dtls13"; 
#endif
    ...
    if (!WPACKET_init_static_len(&pkt, hkdflabel, sizeof(hkdflabel), 0)
            ...
            || !WPACKET_memcpy(&pkt, SSL_IS_DTLS(s) ? label_prefix_dtls : label_prefix, sizeof(label_prefix) - 1)
 ...
 * success  0 on failure.
 */
int tls13_derive_snkey(SSL *s, const EVP_MD *md, const unsigned char *secret,
                     unsigned char *snkey, size_t snkeylen)
{
    static const unsigned char snkeylabel[] = "sn";

    return tls13_hkdf_expand(s, md, secret, snkeylabel, sizeof(snkeylabel) - 1,
                             NULL, 0, snkey, snkeylen, 1);
}
                                    ...
                                    EVP_CIPHER_CTX *sn_ciph_ctx)
{
    ...
    if (!tls13_derive_key(s, md, secret, key, keylen)
            ...
            || (SSL_IS_DTLS(s) && !tls13_derive_snkey(s, md, secret, snkey, keylen))) {
        /* SSLfatal() already called */
        goto err;
    }
    ...
    if (SSL_IS_DTLS(s))
    {
        if (s->s3->tmp.new_cipher->algorithm_enc & (SSL_AES128GCM | SSL_AES128CCM | SSL_AES128CCM8))
          sn_ciph = EVP_aes_128_ecb();
        else if (s->s3->tmp.new_cipher->algorithm_enc & SSL_AES256GCM)
          sn_ciph = EVP_aes_256_ecb();
        else if (s->s3->tmp.new_cipher->algorithm_enc & (SSL_SM4GCM | SSL_SM4CCM))
          sn_ciph = EVP_sm4_ecb();
        else if (s->s3->tmp.new_cipher->algorithm_enc & SSL_CHACHA20POLY1305)
          sn_ciph = EVP_chacha20();
        else
          sn_ciph = NULL;
        
        if (EVP_CipherInit_ex(sn_ciph_ctx, sn_ciph, NULL, snkey, NULL, 1) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_DERIVE_SECRET_KEY_AND_IV,
                        ERR_R_EVP_LIB);
            goto err;
        }
        EVP_CIPHER_CTX_set_padding(sn_ciph_ctx, 0); /* DTLS13 sequence number encryption, not need padding */
    }
...
int tls13_change_cipher_state(SSL *s, int which)
{
  ...
  static const unsigned char early_exporter_master_secret[] = {0x65, 0x20, 0x65, 0x78, 0x70, 0x20,  /* master*/  0x6D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x00};
    ...
    EVP_CIPHER_CTX *sn_ciph_ctx = NULL;
    ...
    if (which & SSL3_CC_READ) {
        ...
        if (SSL_IS_DTLS(s))  /* DTLS1.3 */
        {
            if (s->enc_read_sn_ctx != NULL) {
                EVP_CIPHER_CTX_reset(s->enc_read_sn_ctx);
            } else {
                s->enc_read_sn_ctx = EVP_CIPHER_CTX_new();
                if (s->enc_read_sn_ctx == NULL) {
                    SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                SSL_F_TLS13_CHANGE_CIPHER_STATE, ERR_R_MALLOC_FAILURE);
                    goto err;
            ...
            sn_ciph_ctx = s->enc_read_sn_ctx;
            if (which & SSL3_CC_EARLY)
              dtls1_reset_seq_numbers(s, SSL3_CC_READ, 1);
            else if (which & SSL3_CC_HANDSHAKE)
              dtls1_reset_seq_numbers(s, SSL3_CC_READ, 2);
            else
              dtls1_reset_seq_numbers(s, SSL3_CC_READ, -1);
        }
        else
        ...
        if (SSL_IS_DTLS(s))  /* DTLS1.3 */
        {
            if (s->enc_write_sn_ctx != NULL) {
                EVP_CIPHER_CTX_reset(s->enc_write_sn_ctx);
            } else {
                s->enc_write_sn_ctx = EVP_CIPHER_CTX_new();
                if (s->enc_write_sn_ctx == NULL) {
                    SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                                SSL_F_TLS13_CHANGE_CIPHER_STATE, ERR_R_MALLOC_FAILURE);
                    goto err;
            ...
            sn_ciph_ctx = s->enc_write_sn_ctx;

            if (which & SSL3_CC_EARLY)
              dtls1_reset_seq_numbers(s, SSL3_CC_WRITE, 1);
            else if (which & SSL3_CC_HANDSHAKE)
              dtls1_reset_seq_numbers(s, SSL3_CC_WRITE, 2);
            else
              dtls1_reset_seq_numbers(s, SSL3_CC_WRITE, -1);
        }
        else
