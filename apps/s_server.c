...
typedef enum OPTION_choice {
    ...
    OPT_NOCOOKIE, OPT_ID_PREFIX, OPT_SERVERNAME, OPT_SERVERNAME_FATAL,
...
const OPTIONS s_server_options[] = {
    ...
    {"cert_enc", OPT_CERT_ENC, '<', "Encrypto Certificate file to use when ciphersuit is CNSM"},
#endif
    ...
    {"key_enc", OPT_KEY_ENC, 's',
     "Encypto Private Key if not in -cert when ciphersuit is CNSM"},
#endif
    ...
    {"nocookie", OPT_NOCOOKIE, '-', "no cookies exchange"},
...
#ifndef OPENSSL_NO_CNSM
    const char *s_cert_enc_file = NULL, *s_key_enc_file = NULL;
#endif
...
#ifndef OPENSSL_NO_CNSM
    if (s_cert_enc_file && SSL_CTX_use_certificate_file(ctx, s_cert_enc_file, SSL_FILETYPE_PEM) <= 0)
    {
        ERR_print_errors(bio_err);
        goto end;
    }
    if (s_key_enc_file && SSL_CTX_use_PrivateKey_file(ctx, s_key_enc_file, SSL_FILETYPE_PEM) <= 0)
    {
        ERR_print_errors(bio_err);
        goto end;
    }
...
# endif
            ...
            /* disable cookie exchange for dtls13 */
            if (!nocookie)
